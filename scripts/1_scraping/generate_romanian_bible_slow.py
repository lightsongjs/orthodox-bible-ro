"""
Extract the entire Romanian Orthodox Bible - SLOW VERSION
Uses Romanian book names for folders and English names for pericope matching
OPTIMIZED FOR RATE LIMITING: 2s delays, only 2 workers
"""
import os
import json
import time
...
def safe_print(message):
    """Thread-safe print function with UTF-8 support"""
    with print_lock:
        print(message)
...
def scrape_and_split_book_v2(romanian_name, book_info, pericope_structure_all, output_base_dir, name_mapping):
    ...
    try:
        ...
        for chapter_num in sorted([int(k) for k in book_structure.keys()]):
            # Scrape Romanian chapter with LONGER delay to avoid rate limiting
            romanian_verses = scrape_romanian_chapter(book_id, chapter_num, delay=2.0)
...
def extract_romanian_bible_v2(max_workers=2):
    """
    Extract entire Romanian Bible using Romanian book names - SLOW VERSION

    Args:
        max_workers: Number of parallel workers (default: 2 - reduced for rate limiting)
    """
    print("="*80)
    print("EXTRACTING COMPLETE ROMANIAN ORTHODOX BIBLE - SLOW VERSION")
    print("Using Romanian book names for folders")
    print("2 second delay between requests, 2 parallel workers")
    print("="*80)
    print()
...
if __name__ == "__main__":
    # Use only 2 workers to avoid rate limiting
    extract_romanian_bible_v2(max_workers=2)
